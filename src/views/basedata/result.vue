  ...
  methods: {
    ...
    //确认修改
      ...
      if (this.sign == 1) {
        ...
        url = "admin/SurveyReport/updateResult";
      }
      if (this.sign == 0) {
        url = "admin/SurveyReport/addResult";
      }
    ...
    del(row) {
      ...
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          net
            .request(
              "admin/SurveyReport/delectResult",
              "post",
              { id: row.id },
              {}
            )
    ...
    getUserList(param, data) {
      net
        .request("admin/SurveyReport/queryResultPage", "post", param, null)
        .then(res => {
          if (res.retcode == 1) {
