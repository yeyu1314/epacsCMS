  ...
  methods: {
    drawLine(index, id, serverData) {
      ...
      function getTable(opt, typeName) {
        ...
        var table = '<table style="width:calc(100vw - 300px)"   ><thead>';
        table +=
          '<tr style="height:36px;line-height:36px;background-color:#409eff;">' +
          "<th>" +
          typeName +
          "</th>";
        for (var y = 0; y < series.length; y++) {
          table += "<th>" + series[y].name + "</th>";
        }
        table +=
